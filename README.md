...
```tsx
// Simple example: measure a box and position a label below it before paint
function MeasuredLabel({ text }: { text: string }) {
  ...
  return (
    <div>
      ...
      <p style={{ marginTop: height }}>Height: {height}px</p>
    </div>
  )
}
```

**Rule of thumb:** If the user would see a flash of wrong layout, try `useLayoutEffect`. Otherwise prefer `useEffect`.
...
function ProductList({ products, search }: { products: Product[]; search: string }) {
...
**Rule:** Don’t wrap every calculation in `useMemo` — use it when the work is costly or when a stable reference matters for performance.
...
`useCallback` remembers a **function** between renders. It returns the same function reference until its dependencies change. This helps avoid unnecessary re-renders when passing callbacks to memoized children.

**When to use:** Passing handlers to `React.memo` components, or when a function is a dependency of `useEffect` / `useMemo`.

```tsx
// Simple example: stable toggle handler passed to each list item
function TodoList() {
...
## 10. Composition: children and wrappers
...
| **useLayoutEffect**   | Run DOM reads/writes after update, before paint (avoids flicker). |
